...
public class SearchCDRTools {
	public static final Log LOG = LogFactory.getLog(SearchCDRTools.class.getName());
...
//LOG.info("cdr Time"+simpleDateFormat.format(date));
		if(startTime.length!=endTime.length)
		{
			result=false;
			// LOG.info("query conditons'time pair wrong!");
		}
...
//LOG.info("Start Time"+simpleDateFormat.format(date));
//date.setTime((Long.valueOf(endTime[i])*1000L));
//LOG.info("End Time"+simpleDateFormat.format(date));
					result=true;
					break;
				}
		}
...
//	    	LOG.info("主叫:"+cdr.getCalling_number());

//	    	LOG.info("被叫:"+cdr.getCalled_number());

//	    	LOG.info("cdr_type:"+cdr.getCdr_type());

//	    	LOG.info("网元类型:"+cdr.getNetElem());

//	    	LOG.info("网元设置数据:"+cdr.getNetElemId());

//	    	LOG.info("局向设置数据:"+cdr.getOpc_dpc());
...
//				LOG.info("开始时间:" + list[i]);

//			}

//			for (int i = 0; i < list1.length; i++) {

//				LOG.info("结束时间:" + list1[i]);
...
////	    	LOG.info("开始数目:"+cdr.getBeginNum());

////	    	LOG.info("查询数目:"+cdr.getOnceCount());

//	    	LOG.info("表名:"+cdr.getTableName());

//	    	LOG.info("表类型:"+cdr.getTableType());

//	    	LOG.info("查询log:"+cdr.getIndexLog());

	    	

	    	String ab = "11111";

	    	LOG.info(ab.getBytes().length);
